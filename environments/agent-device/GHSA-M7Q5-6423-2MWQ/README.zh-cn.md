...
状态：草稿。当前仓库内机制复现尚未执行，不能据此宣称已复现；本目录不构成漏洞确认。
...
`verify.py` 独立读取 `facts.json` 与 `observation.json`，校验上下文、哈希和四组对照：漏洞版两个认证请求、修复版拒绝且零请求、benign 零请求以及目标可执行。仓库内尚未运行，状态为 `not_run`。
曾在容器外完成过同样的上游构建与无害对照，这只能说明配方路径方向合理，不能替代容器内三轮验收。
